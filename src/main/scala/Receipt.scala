...
" <br/>"
  ...
  val kk = Buffer(pizza,Omlette, puuro)
   ...
   , "salt"-> 100, "black pepper" -> 30, "ginger"-> 90, "water" -> 10000, "oatmeal" -> 900, "pasta" -> 1000,
  ...
  val puuroo = Map ("water" -> 500, "oatmeal" -> 400 , "milk"-> 500, "salt"-> 10 )
  val pastaa = Map ("pasta" -> 200 , "feta"-> 200, "oil"-> 100 , "cherry tomato" -> 400, "basilica"-> 20, "salt"-> 10,  "black pepper" -> 10)
  val veganpuurorsepti = Map ("water" -> 500, "oatmeal" -> 400 , "salt"-> 10 )
  ...
  val ilmanAllergiapuurooResepti = Map ( "oatmeal" -> 400 , "lactose free milk"-> 500 )
 ...
 "without allergies omlette"-> ilmanAllergiaOmeletteResepti, "without allergies porridge"-> ilmanAllergiapuurooResepti,"without allergies fetapasta" ->ilmanallergiapastaResepti,
  ...
  def makefood (ruoka: String)={
    ...
    for {
      ...
      if(puuttuvat.toList.isEmpty){
         text = " " + kuvaukset(ruoka)+ "<br>"

      }
    else{
         text = text + "You cannot use this recipe because you have run out  the raw material, please check raw materia section whats is missing<br>"
    }
 ...
 def maketext (ruoka: String)={
   var text = "<html>"
    for {
      ...
      text = text + reseptinAine + " " +"the amount is a suitable amount to use in the recipe<br/>"
       ...
       val puuttuu =   reseptinMaara - ruokakaappi(reseptinAine)
      text = text + "out of" + " " + reseptinAine + "  " + "and need at least" + " " + puuttuu + "g"  + " "+ reseptinAine + "<br>"
}
    }
    text += "</html>"
    text
  ...
  def addRawMaterials(ruoka: String)={
   var text = "<html>"
    for {
      (reseptinAine, reseptinMaara) <- reseptit(ruoka)
    } {
